...
import { bookFlight } from "../../services/Flightservices";

const FlightSummary = () => {
  const { state }  = useLocation();
  const navigate   = useNavigate();
  const { flight, search, fare, passengers, contact } = state || {};

  const [loading, setLoading] = useState(false);
  const [error, setError]     = useState("");

  if (!flight) { navigate("/flights"); return null; }

  const baseFare    = fare * passengers.length;
  const airportTax  = 450 * passengers.length;
  const fuelSurcharge = Math.round(baseFare * 0.06);
  const gst         = Math.round((baseFare + airportTax + fuelSurcharge) * 0.05);
  const total       = baseFare + airportTax + fuelSurcharge + gst;
  ...
  const handlePay = async () => {
    ...
    try {
      const result = await bookFlight({
        flightId:    flight.flightId || flight.flightNo,
      ...
      if (result.success) {
        navigate("/flights/confirmation", { state: { booking: result.booking, flight, search } });
      } else {
  ...
  const Row = ({ label, value, bold, accent }) => (
    <div className={`flex justify-between items-center py-2.5 border-b border-gray-50 ${bold ? "border-b-0 pt-3" : ""}`}>
      ...
      <span className={`text-sm font-medium ${accent ? "text-blue-600" : bold ? "text-gray-800" : "text-gray-700"}`}>{value}</span>
    </div>
  );
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="min-h-screen bg-gray-50">
      ...
      <div className="relative overflow-hidden pb-14 pt-5 px-5"
        ...
        <p className="relative text-sm mt-0.5" style={{ color: "rgba(255,255,255,0.6)" }}>Review before paying</p>
      </div>
      ...
      <div className="px-4 pb-8 -mt-5">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-4 mb-3">
          <div className="flex items-start justify-between mb-3">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">{flight.flightNo} · {search.cabin}</p>
            </div>
            <span className="text-xs font-medium px-2 py-1 rounded-full bg-blue-50 text-blue-800">{search.date}</span>
          </div>
          ...
          {passengers.map((p, i) => (
            <div key={i} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-b-0">
              <div>
                ...
                <p className="text-xs text-gray-400">{p.age} yrs · {p.gender} · {p.seat} seat · {p.meal}</p>
              </div>
              <p className="text-xs text-gray-400">{p.idType}</p>
            </div>
          ))}
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-4 mb-3">
          ...
          <Row label="Airport taxes"                      value={`₹${airportTax.toLocaleString("en-IN")}`} />
          <Row label="Fuel surcharge (6%)"                value={`₹${fuelSurcharge.toLocaleString("en-IN")}`} />
        ...
        {/* Contact */}
        <div className="bg-white rounded-2xl border border-gray-100 p-4 mb-3">
          <p className="text-xs font-medium uppercase tracking-widest text-gray-400 mb-2">Boarding pass sent to</p>
